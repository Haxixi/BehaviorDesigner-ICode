...
namespace ICode{
	public static class FsmUtility {
		...
		public static ExecutableNode Copy(ExecutableNode original){
			...
			FieldInfo[] fields = original.GetType ().GetFields (BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);//.Where(x=> x.IsPublic || x.IsPrivate || x.;
		...
		private static object CopyFields(object source){
			...
			if (type.IsValueType() || type == typeof(string) || type == typeof(GameObject) || typeof(UnityEngine.Object).IsAssignableFrom (type) && !typeof(FsmVariable).IsAssignableFrom (type)) {
				...
				/*if(source is FsmVariable && (source as FsmVariable).IsShared){
					return source;
				}*/
				//Debug.Log(source);
				ScriptableObject dest = (ScriptableObject)ScriptableObject.Instantiate (source as ScriptableObject);
				dest.hideFlags = HideFlags.HideInHierarchy;
				FieldInfo[] fields = type.GetFields (BindingFlags.Public | BindingFlags.Instance);
				...
				for (int i=0;i<properties.Length;++i) {
					...
					if (!property.CanWrite || !property.PropertyType.IsSubclassOf(typeof(ScriptableObject))) {
						continue;
					}
					object propertyValue = property.GetValue (source, null);
					if (propertyValue == null) {
