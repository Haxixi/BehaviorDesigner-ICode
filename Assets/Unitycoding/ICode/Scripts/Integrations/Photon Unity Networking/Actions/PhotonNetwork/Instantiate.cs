...
namespace ICode.Actions.Photon{
	...
	public class Instantiate : StateAction {
		...
		private void DoInstantiate(){
			store.Value = PhotonNetwork.Instantiate (prefabName.Value, FsmUtility.GetPosition(target, position), target.Value != null && euler.IsNone?target.Value.transform.rotation:Quaternion.Euler(euler.Value), group.Value);
		}
	}
}
#endif