...
namespace ICode.Actions.PlayMaker{
	...
	public class SetVector2 : StateAction {
		...
		private void DoSet(){
			...
			HutongGames.PlayMaker.FsmVector2 fsmVar= fsm.FsmVariables.FindFsmVector2(variableName.Value);
			if (direction == SetDirection.ToPlayMaker) {
				if(fsmVar != null && icodeVariable != null && icodeVariable is FsmVector2){
					fsmVar.Value=(Vector2)icodeVariable.GetValue();
				}
			} else {
				if(fsmVar != null && icodeVariable != null && icodeVariable is FsmVector2){
