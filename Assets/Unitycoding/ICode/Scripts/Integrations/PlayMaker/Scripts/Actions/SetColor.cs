...
namespace ICode.Actions.PlayMaker{
	...
	public class SetColor : StateAction {
		...
		private void DoSet(){
			...
			HutongGames.PlayMaker.FsmColor fsmVar= fsm.FsmVariables.FindFsmColor(variableName.Value);
			if (direction == SetDirection.ToPlayMaker) {
				if(fsmVar != null && icodeVariable != null && icodeVariable is FsmColor){
					fsmVar.Value=(Color)icodeVariable.GetValue();
				}
			} else {
				if(fsmVar != null && icodeVariable != null && icodeVariable is FsmColor){
