...
namespace ICode.Actions.PlayMaker{
	...
	public class SetGameObject : StateAction {
		...
		private void DoSet(){
			...
			HutongGames.PlayMaker.FsmGameObject fsmVar= fsm.FsmVariables.FindFsmGameObject(variableName.Value);
			if (direction == SetDirection.ToPlayMaker) {
				if(fsmVar != null && icodeVariable != null && icodeVariable is FsmGameObject){
					fsmVar.Value=(GameObject)icodeVariable.GetValue();
				}
			} else {
				if(fsmVar != null && icodeVariable != null && icodeVariable is FsmGameObject){
